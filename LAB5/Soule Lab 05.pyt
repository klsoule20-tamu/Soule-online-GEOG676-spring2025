#Kaleb Soule Lab 05 Resubmittal
...
class tool(object):
    ...
    def execute(self, parameters, messages):
        ...
        arcpy.Copy_mangement(buildings_campus, buildings)

        spatial_ref = arcpy.Describe(buildings).spatialReference
        arcpy.Project_management(garage_points, gdb_path + '\Garage_Points_reprojected', spatial_ref)

        buffer_distance = int(parameters[5].value)
        garageBuffered = arcpy.Buffer_analysis(gdb_path + 'Garage_Points_reprojected', gdb_path + '\Garage_Points_reprojected', 150)
